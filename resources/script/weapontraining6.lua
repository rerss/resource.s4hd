...
function Tutorial_Step_First(step)
	...
	-- 캐릭터 무기 설정.
	TutorialSystem2:SetWeaponSlot(ACTOR.ME, 0, WEAPON.MINDENE, 0 ); -- 마인드에너지
	TutorialSystem2:SetWeaponSlot(ACTOR.ME, 1, WEAPON.MINDSHO, 0 ); -- 마인드에너지
...
---------------------------------------------------------------------------	
	if step == step_c then
		...
		if TutorialSystem2:GetHP(ACTOR.OURS_1) >= 50 then
			...
			TutorialSystem2:ChangeMessageWindow(MENT.CHARACTER1,0);
			TutorialSystem2:SetActorPos(
			POS.ENEMY_SPAWN._02[1],POS.ENEMY_SPAWN._02[2],POS.ENEMY_SPAWN._02[3], ACTOR.OURS_1);
