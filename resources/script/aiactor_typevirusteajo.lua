...
function TypeVirusTeajo_CostumeSetting()
	...
	AIActor:SetCostume( 1020065, 0 ); --태조 상의
	AIActor:SetCostume( 1030037, 0 ); --태조 하의
	AIActor:SetCostume( 1040033, 0 ); --와일드 블랙 너클
	AIActor:SetCostume( 1050029, 0 ); --스페이스 부츠
	AIActor:SetCostume( 1060000, 0 ); --바이러스
...
function TypeVirusTeajo_AddAttack1AIActorState()
	...
	state:SetNextStateID( AAINPUT_NORMAL, AASTATE_NORMAL );
	state:SetNextStateID( AAINPUT_FALL, AASTATE_FALL );
	-- 난이도가 너무 어려울 때 데미지 딜레이가 발생하면 공격이 캔슬되도록 변경한다.
	--state:SetNextStateID( AAINPUT_DAMAGE, AASTATE_DAMAGE );
	state:SetNextStateID( AAINPUT_COUNTERATTACK_DAMAGE, AASTATE_COUNTERATTACK_DAMAGE );
	state:SetNextStateID( AAINPUT_DEATH, AASTATE_DEATH );
	state:SetNextStateID( AAINPUT_STUN, AASTATE_STUN );
	...
	state:RegisterSound( "Resources/Sound/Acade/virus_voice_man_attack_3.ogg", "Bip01", 0, 2500.0 );
	...
	StateFactory:AddCounterSwordCounterAttacks( state, 30.0, 600.0 );
