...
function CreateMoveAttack( monster )
	...
	moveAttackState:SetMoveSpeedMultiplier( 2.0 );
...
function SetAnimation( monster )
	...
	if nil ~= state then
		...
		--state:AddSound( 200, "Resources/Sound/challenge_arcade/king_damage.ogg" );
	end
	...
	if nil ~= state then
		...
		state:AddSequence( 0, "crown_Dummy", "Resources/Effects/pve_monster_king_crown.seq" ); -- starttime, node, sequence
...
function Update( timeDelta, monster )	
	...
	-- if false == monster:IsWaitTimeValid() then
		...
		userFindDistance = 1800;		-- 유저 찾는 거리
