...
---------------------------------------------------------------------------
	...
	TutorialSystem2:SetWeaponSlot(ACTOR.ENEMY_1, 0, WEAPON.PLA , 0 ); -- 플라즈마스워드.
	...
	MAKEVECTOR3(POS.ENEMY_SPAWN._02[1] ,POS.ENEMY_SPAWN._02[2],POS.ENEMY_SPAWN._02[3]), 0, 0, 0 , "League");
	TutorialSystem2:SetWeaponSlot(ACTOR.ENEMY_2, 0, WEAPON.HOMING , 0 ); -- 플라즈마스워드.
...
----------------------------------------------------------------------------------
	if step == step_c then
	TutorialSystem2:ChangeMessageWindow(MENT.HIDE,	0); -- 가이드 UI HIDE
	TutorialSystem2:SetTimer(0,30000); -- 30초이내에 메탈릭스킬 사용?
...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		--타이머가 끝이나지않았고. 스킬을 사용하고있을때.
		if	TutorialSystem2:GetTimerState(0) == TIMER.DOING and
			TutorialSystem2:GetMatchActorState(ACTOR.ME,STATE_SKILL_SHIELD) == true then
			TutorialSystem2:SetStep(step+1);
		end
...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		if TutorialSystem2:IsDoingState(ACTOR.ME, STATE_SKILL_SHIELD ) == true and
			...
			TutorialSystem2:ElapsedTimeFromAttack(ACTOR.ME) < 300 then
...
--------------------------------------------------------------------------------------
if step == step_c then
	if TutorialSystem2:IsActorLocated( ACTOR.ENEMY_1, TutorialSystem2:GetActorPosX(ACTOR.ME) , 
		TutorialSystem2:GetActorPosY(ACTOR.ME) , TutorialSystem2:GetActorPosZ(ACTOR.ME) , 400 ) == true then
...
-------------------------------------------------------------------------------------
	if step ==step_c then
		...
		--TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_SKILL_SHIELD, 0);
		--TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0);
		TutorialSystem2:SKillOff(ACTOR.ME,ONOFF_SHIELD);
		TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0);
		TutorialSystem2:SetStep(ReturnStep);
