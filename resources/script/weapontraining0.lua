...
function Tutorial_Step_First(step)
	...
	-- 캐릭터 무기 설정.
	TutorialSystem2:SetWeaponSlot(ACTOR.ME, 0, WEAPON.CANO, 0 ); -- 캐노네이드
	TutorialSystem2:SetWeaponSlot(ACTOR.ME, 1, WEAPON.KATA, 0 ); -- 카타나
...
---------------------------------------------------------------------------	
	if step == step_c then
		...
		-- 적 캐릭터 2방 맞음 여부.
		if  TutorialSystem2:GetPlayerAttackCount(Weapon1State) >= 2 and
			TutorialSystem2:GetDamageCounter(ACTOR.ENEMY_1) >= 2 then
...
---------------------------------------------------------------------------	
	...
	ment_id = ment_id + 1;
	step_c = Tutorial_Step_PageScript(step, step_c, ment_id, char_time); 
-- 	카타나의 Charge공격 방법은  [마우스 우클릭]을 누르고있는 동안 Charge를하고 손을 때는순간 공격을 실행합니다.
	--------------------------------------------------------------------------
	if step == step_c then
	TutorialSystem2:ChangeMessageWindow(MENT.HIDE,	0); -- 가이드 ui hide
	TutorialSystem2:SetTimer(0,60000); -- 60 초 제한 타이머 설정.
