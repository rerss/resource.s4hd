...
function CreateAttack1( monster )	--돌진 공격, 노멀에서는 사용하지 않음.
    ...
    Attack1:InitAttack2( MAKEBOX( MAKEVECTOR3(0,180,0), MAKEVECTOR3(0,0,-1), 320*1, 240, 480 ), 1, 1, false  );
...
function SetAnimation( monster )	-- 애니메이션 등록
	...
	if nil ~= state then
		...
		state:AddSound( 200, "Resources/Sound/challenge_arcade/AI_damage.ogg" );	-- 사운드 추가함!
	end
	...
	if nil ~= state then
		...
		state:AddSound( 200, "Resources/Sound/challenge_arcade/destroy.ogg" );	-- 사운드 추가함!
	end
	...
	if nil ~= state then
		...
		state:AddSound( 200, "Resources/Sound/challenge_arcade/knight_attack.ogg" );	-- 사운드 추가함!
	end
...
function ActorCheck( timeDelta, monster )
	...
	--{	
		...
		--{
			...
			monster:SetWaitTime( 820 );	-- 해당 함수 실행 후 820 동안 대기
...
function Update( timeDelta, monster )	
	...
	-- 공격 가능 거리 		
	attackMaxDistance = 400;		-- 공격 최대거리( 이 거리보다 멀면 공격하지 않고 유저에게 다가간다.)
