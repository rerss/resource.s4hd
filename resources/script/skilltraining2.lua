...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		if	TutorialSystem2:GetTimerState(0) == TIMER.DOING and
			TutorialSystem2:GetMatchActorState(ACTOR.ME,STATE_SKILL_METALIC) == true then
			TutorialSystem2:SetReturn(false);
			TutorialSystem2:ClearEnableInput();
			TutorialSystem2:SetStep(step+1);
		end
...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		TutorialSystem2:GetActorPosY(ACTOR.ME) , TutorialSystem2:GetActorPosZ(ACTOR.ME) , 400 ) == true then -- 적ai 가 원위치에 왔는지 안왔는지 체크.	
