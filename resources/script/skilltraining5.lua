...
---------------------------------------------------------------------------
	...
	TutorialSystem2:SetWeaponSlot(ACTOR.ENEMY_1, 0, WEAPON.HOMING , 0 ); -- 호밍라이플.
	...
	TutorialSystem2:SetWeaponSlot(ACTOR.ENEMY_2, 0, WEAPON.HOMING , 0 ); -- 호밍라이플.
...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		if	TutorialSystem2:GetTimerState(0) == TIMER.DOING and
			TutorialSystem2:GetMatchActorState(ACTOR.ME,STATE_SKILL_STEALTH) == true then
			TutorialSystem2:LockGamePlayInput(false);
...
--------------------------------------------------------------------------------
	if step == step_c then
		...
		if TutorialSystem2:IsActorLocated(ACTOR.ME, 1199, 2925, -4577, 300) == true 
		...
		if	TutorialSystem2:IsActorLocated(ACTOR.ME,
			POS.GOAL[1], POS.GOAL[2], POS.GOAL[3], 300 ) == true and 
			TutorialSystem2:GetData(0) == 1 then
			TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0);
			TutorialSystem2:Goal();
			TutorialSystem2:SetStep(step+1);
		end
			
		if TutorialSystem2:GetTimerState(0) == TIMER.END or 
			TutorialSystem2:GetHP(ACTOR.ME) <= 10 then
			TutorialSystem2:LockGamePlayInput(true);
			TutorialSystem2:PushActorCommandState(ACTOR.ME, STATE_NORMAL, 0);
			TutorialSystem2:SetReturn(true);
			TutorialSystem2:ClearAllActorCommands(ACTOR.ENEMY_2);
